...
Original file is located at
    https://colab.research.google.com/drive/1O-nh3NtXkVe1sh-qXv6pbzvxefFVswjZ
...
class CbowDataset(data.Dataset):
    ...
    def get_context_vector(self, idx):
        ...
        return torch.tensor([self.word2idx[word] for word in context_words])

    def get_target_index(self, idx):
        '''
        Returns the index of the target word (as type int) of the training example at index idx.
        '''
        target_word = self.training_examples[idx][1]
        return self.word2idx[target_word]
...
class CbowModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hidden_out = F.relu(self.hidden_layer(avg_embedding))
...
BATCH_SIZE = 100 # You may change the batch size if you'd like
...
def train_cbow_model(model, num_epochs, data_loader, optimizer, criterion, save_path='cbow_model.pth'):
...
class TextDataset(data.Dataset):
    ...
    def convert_text(self):
        ...
        for label, review in self.examples:
          ls = []
          for word in review:
            word = word.lower()
            if word in self.word2idx:
              ls.append(self.word2idx[word])
            else:
              ls.append(self.word2idx[CNN_UNK])
          ls.append(self.word2idx[CNN_END])
          self.textual_ids.append(ls)
    ...
    def get_text(self, idx):
        ...
        '''
        sentence = self.examples[idx][1]
        indices = []
        for word in sentence:
          indices.append(self.word2idx[word.lower() if word.lower() in self.word2idx else CNN_UNK])
        indices.append(self.word2idx[CNN_END])

        if len(indices) < self.max_len:
          for i in range(self.max_len - len(indices)):
            indices.append(self.word2idx[CNN_PAD])
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, dropout, num_classes, pad_idx):
        ...
        self.embeddings = nn.Embedding(vocab_size, embed_size, pad_idx)
        ...
        self.convolutions = nn.ModuleList([nn.Conv1d(embed_size, out_channels, filter_height, stride) for filter_height in filter_heights])

        # Create a dropout layer (nn.Dropout) using
    ...
    def forward(self, texts):
        ...
        for conv in self.convolutions:
            conv_out = conv(texts)  # [batch_size, out_channels, new_seq_len]
            conv_out = F.relu(conv_out)
            conv_out = F.max_pool1d(conv_out, conv_out.shape[2])  # [batch_size, out_channels, 1]
            conv_out = conv_out.squeeze(2)  # [batch_size, out_channels]
            conv_outputs.append(conv_out)
        texts = torch.cat(conv_outputs, dim=1)
        ...
        # Everything happens on a batch simultaneously hence you hav`e that additional batch_size as the first dimension

        # Apply dropout
        texts = self.dropout(texts)

        # Pass your output through the linear layer and return its output
        #   Resulting shape: [batch_size, num_classes]
        return self.linear(texts)
